...
const SignatureModal: React.FC<SignatureModalProps> = ({
  ...
  const handleSaveSignature = () => {
    if (!signatureName.trim()) {
      Alert.alert('Error', 'Please enter a name for the signature');
      return;
    }
    // Create a simple text signature as base64 data URL
    const canvas = `data:text/plain;base64,${btoa(`Signed by: ${signatureName.trim()}\nDate: ${new Date().toLocaleDateString()}`)}`;
    onSave(canvas);
    setSignatureName('');
    onClose();
  };
