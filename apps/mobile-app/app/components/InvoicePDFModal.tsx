...
const InvoicePDFModal: React.FC<InvoicePDFModalProps> = ({
  ...
  const { data: workOrders, isLoading: workOrdersLoading } = useQuery({
    ...
    queryFn: async () => {
      const response = await apiRequest(
        "GET",
        `/api/tickets/${invoice.ticketId}/work-orders`,
      );
        ...
        <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
          <View style={styles.pdfContainer}>
                  ...
                  {ticket?.locationId && (
                    <View style={styles.serviceLocationBox}>
                      <Text style={styles.serviceLocationTitle}>
                        Service Location:{" "}
                        {ticket.location?.name || "Service Location"}
                      </Text>
                ...
                <View style={styles.ticketBox}>
                  <View style={styles.ticketRow}>
                    ...
                    <Text style={styles.ticketValue}>
                      {ticket.ticketNumber || invoice.ticketId}
                    </Text>
                  </View>
                  <View style={styles.ticketRow}>
                ...
                <View style={styles.table}>
                  ...
                  {workOrders.map((workOrder: any, index: number) => {
                    ...
                    const partsCost = parts.reduce(
                      (sum: number, part: any) =>
                        sum +
                        parseFloat(part.cost || 0) *
                          parseInt(part.quantity || 0),
                      0,
                    );

                    const laborHours = parseFloat(
                      workOrder.totalHours || workOrder.hours || 0,
                    );
                    ...
                    return (
                      <View key={workOrder.id || index} style={styles.tableRow}>
                        ...
                        <View style={[styles.tableCell, { flex: 2 }]}>
                          <Text
                            style={[styles.tableCellText, { fontSize: 12 }]}
                          >
            ...
            <View style={styles.paymentSection}>
              ...
              {!invoice.paidAt && (
                <Text style={styles.paymentText}>Payment due upon receipt</Text>
              )}
            </View>
          </View>
        </ScrollView>
