...
export default function DashboardScreen() {
  ...
  const handleTicketAction = (ticket: Ticket, action: "accept" | "reject") => {
    ...
    router.push({
      pathname: `/ticket/${ticket.id}?action=${action}`,
      params: { ticket: JSON.stringify(ticket) },
    });
  };
              ...
              <Text style={styles.statValue}>
                {
                  tickets.filter(
                    (t) =>
                      t.organizationId === selectedOrganization?.id &&
                      t.status === "completed",
                  ).length
                }
              </Text>
              ...
              <Text style={styles.statValue}>
                {
                  tickets.filter(
                    (t) =>
                      t.maintenanceVendorId === selectedVendor?.id &&
                      t.status === "completed",
                  ).length
                }
              </Text>
    ...
    <SafeAreaView style={styles.container}>
      ...
      >
        <ScrollView
          ...
          showsVerticalScrollIndicator={false}
        >
          {currentView === "dashboard" && user?.role === "root"
            ? renderRootDashboard()
