...
export class GoogleCalendarService {
  ...
  formatTime = (time: string | undefined, fallback: string) => {
    if (!time) return fallback;
    return time.match(/^\d{2}:\d{2}$/) ? time + ":00" : time; // e.g. "06:00" ➝ "06:00:00"
  };
  // Helper function to format time strings for Google Calendar API
    ...
    try {
      ...
      if (event.isAllDay) {
        ...
        const startTime = event.startTime
          ? this.formatTime(event.startTime, "00:00:00")
          : "00:00:00";
        const endTime = event.endTime
          ? this.formatTime(event.endTime, "23:59:59")
          : "23:59:59";
      ...
      console.log(
        `Created Google Calendar event: ${event.title} (${response.data.id})`,
      );
  ...
  async syncFromGoogle(
    ...
    try {
      ...
      console.log(
        `Syncing Google Calendar events from ${timeMin.toISOString()} to ${timeMax.toISOString()}`,
      );
